...
export default class HotelDatepicker {
	constructor(input, options) {
		...
		this.setValue = opts.setValue || function (s) {
			const startHour = document.getElementById(input.id + 'StartHourValue').value;
			const startMin = document.getElementById(input.id + 'StartMinValue').value;
			const endHour = document.getElementById(input.id + 'EndHourValue').value;
			const endMin = document.getElementById(input.id + 'EndMinValue').value;
			const dateComponents = s.split(this.separator);
			const dateTimeString = dateComponents[0] + ' ' + this.pad(startHour, 2) + ':' + this.pad(startMin, 2) + this.separator + dateComponents[1] + ' ' + this.pad(endHour, 2) + ':' + this.pad(endMin, 2);
	...
	init() {
		...
		document.getElementById(this.input.id + 'StartHourValue').oninput = (that => {
			return function () {
				that.onTimeChanged(that);
			};
		})(this);

		document.getElementById(this.input.id + 'StartMinValue').oninput = (that => {
			return function () {
				that.onTimeChanged(that);
			};
		})(this);

		document.getElementById(this.input.id + 'EndHourValue').oninput = (that => {
			return function () {
				that.onTimeChanged(that);
			};
		})(this);

		document.getElementById(this.input.id + 'EndMinValue').oninput = (that => {
			return function () {
				that.onTimeChanged(that);
			};
		})(this);
